...
class ShoppingListElement extends React.Component<
  ShoppingListElementProps,
  ShoppingListElementState
> {
  ...
  editItem() {
    ...
    fetch(`${APIURL}/items/edit/${this.props.item.id}`, {
      method: "PUT",
      body: JSON.stringify({
  ...
  checkItem(e: CheckboxChangeEvent) {
    fetch(`${APIURL}/items/update-item/${this.props.item.id}`, {
      method: "PUT",
      body: JSON.stringify({
  ...
  // delete item
  deleteItem() {
    fetch(`${APIURL}/items/delete/${this.props.item.id}`, {
      method: "DELETE",
      headers: new Headers({
        "Content-Type": "application/json",
        Authorization: `${localStorage.getItem("token")}`,
  ...
  render() {
    const menu = (
      <Menu >
        ...
        <Menu.Item key="2">
          <Button className="borderless" onClick={this.deleteItem}>
    ...
    return (
      <div>
        ...
        >
          <Checkbox
            defaultChecked={this.props.item.bought}
          ...
          <Form {...layout} name="myList">
            ...
            >
              <Input
                ...
                onChange={(e) => this.setState({ item_new: e.target.value })}
              />
            </Form.Item>
            <Form.Item
              name={["Quantity"]}
            ...
            >
              <InputNumber
                ...
                onChange={(e) => this.setState({ quantity_new: Number(e) })}
              />
            </Form.Item>
          </Form>
        </Modal>
