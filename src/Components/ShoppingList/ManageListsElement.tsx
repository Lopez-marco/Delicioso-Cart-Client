...
const ManageListElement: React.SFC<ManageListElementProps> = (props) => {
    ...
    function renameList(id: number) {
        fetch(`http://localhost:3001/shopping-list/edit/${id}`, {
            method: 'PUT',
            body: JSON.stringify({
                name: newListName
            }),
    ...
    function deleteList(id: number) {
        fetch(`http://localhost:3001/shopping-list/delete/${id}`, {
            method: 'DELETE',
            headers: new Headers({
                'Content-Type': 'application/json',
                'Authorization': `${localStorage.getItem('token')}`
    ...
    const menu = (
        <Menu>
            <Menu.Item key="1" >
                <Button className='borderless' onClick={editListModal}><EditOutlined /> Rename</Button>
            </Menu.Item>
            <Menu.Item key="2" >
                <Button className='borderless' onClick={deleteListModal}><DeleteOutlined /> Delete</Button>
            </Menu.Item>
        </Menu>
    );
    return (
        <div>
            ...
            >
                <Form {...layout} name="renameList">
                    <Form.Item name={['renameListInput']} label="Rename List" rules={[{ required: true }]}>
